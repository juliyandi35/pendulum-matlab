...
for i = 1:nFrames
    ...
    waitbar(i/nFrames, Fig, ...
        sprintf('正在处�?�帧...%.2f%% (%u/%u)', 100*i/nFrames, i, nFrames));
end
close(Fig)
...
%% 生�?音频
[file, path] = uiputfile({'*.flac';'*.wav'}, '�?存音频文件', 'Audio');
fs = 48e3; % 音频采样率
t = 0:1/fs:dur-1/fs;
kf = 256; % 调频�?��?度
fc = 256; % 载波频率
